...
if(isset($_POST['type']) && $_POST['type']!='' && isset($_POST['p_id']) && $_POST['u_id']>0){
	...
	if($type=='like'){
		if(isset($_COOKIE['like_'.$id])){
			...
			if(isset($_COOKIE['dislike_'.$id])){
				...
				mysqli_query($con,"UPDATE poster set dislike_count=dislike_count-1 where p_id='$id'");
			}
	...
	if($type=='dislike'){
		if(isset($_COOKIE['dislike_'.$id])){
			...
			if(isset($_COOKIE['like_'.$id])){
				setcookie('like_'.$id,"yes",1);
				mysqli_query($con,"UPDATE poster set like_count=like_count-1 where p_id='$id'");
			}
	...
	$row=mysqli_fetch_assoc(mysqli_query($con,"SELECT * from poster where p_id='$id'"));

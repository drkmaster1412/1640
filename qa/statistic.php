...
<body>
    ...
    <div id="main-content" class="container p-5" style="margin-left:30%">
        ...
        if (isset($_POST["sortBtn"])) {
            ...
            $sql .= " ORDER BY like_count $sortOrder";
        }
        ...
        if (isset($_POST["downloadBtn"])) {
            ...
            fputcsv($fp, array('Poster name', 'Poster user', 'like', 'dislike'));
            //Loop through the result set and write each row to the CSV file
            while ($row = $result->fetch_assoc()) {
                fputcsv($fp, array($row['p_name'], $row['p_user'], $row['like_count'], $row['dislike_count']));
            }
        ...
        if ($result->num_rows > 0) {
            //Output table header
            echo "<table border='1'>";
            echo "<tr><th>Poster Name</th><th>Poster user</th><th>like</th><th>dislike</th></tr>";

            //Loop through the result set and output each row as a table row
            while ($row = $result->fetch_assoc()) {
                echo "<tr><td>" . $row['p_name'] . "</td><td>" . $row['p_user'] . "</td><td>" . $row['like_count'] . "</td><td>" . $row['dislike_count'] . "</td></tr>";
            }
